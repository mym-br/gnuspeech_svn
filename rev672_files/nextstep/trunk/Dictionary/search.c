...
/*#define dictionary "/LocalLibrary/TextToSpeech/system/MainDictionary"*/
/*#define dictionary "/Accounts/schock/.speechlibrary/converted_index"*/
#define dictionary "./SortedDictionary"
...
init_dict(systemPath)
const char *systemPath;
{
#define dictionary_name "/SortedDictionary"
char tempPath[256];

        strcpy(tempPath, systemPath);
        strcat(tempPath, dictionary_name);

	fp = NXMapFile(tempPath, NX_READONLY);	/* Map file into memory */
	if (fp==NULL)					/* exit if file not found */
	{
/*		NXLogError("TTS_Server: Cannot open Main Dictionary.");*/
