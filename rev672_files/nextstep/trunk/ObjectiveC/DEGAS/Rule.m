...
char string[1024];
	...
	/*
	 * IF THIS HIT HAS ALREADY BEEN HIT JUST PREVIOUSLY, RETURN TO SAVE TIME (SINCE DISPLAY ALREADY UPDATED)  
	 */
...
- (int)browser:sender fillMatrix:matrix inColumn:(int)column
{
	...
	if (sender == specialEventList)
	{
		return (currentSpecifierPtr->number_of_special_events);
	}
	else
	if (sender == addSpecialEventList)
	{
...
union _split_interval_duration
{
	...
	/*
	 * GET THE SPLIT INTERVAL'S PERCENTAGE RISE OR SLOPE, DEPENDING ON MODE. DIVIDE IN HALF FOR DISTRIBUTION BETWEEN THE TWO
	 * RESULTING INTERVALS, IF RISE;  USE SAME SLOPE FOR BOTH INTERVALS IF IN SLOPE RATIO MODE  
	 */
	if (currentSpecifierPtr->t_interval_mode == FIXED_RISE_MODE)
	...
	/*
	 * DETERMINE SPLIT INTERVAL DURATION, DIVIDE IN HALF FOR FURTHER DISTRIBUTION, KEEPING IN MIND THE MINIMUM VALUES ALLOWED  
	 */
...
t_intervalPtr current_interval_ptr, temp_interval_ptr, new_t_interval();
	...
	if (specifierPtr->t_interval_mode == SLOPE_RATIO_MODE)
	{
		/* ADD THREE INTERVALS  */
		current_interval_ptr = specifierPtr->t_intervalHead = new_t_interval();
		current_interval_ptr->proportional = YES;
...
float old_proportion, proportion_change;
			...
			/*
			 * LIMIT PROPORTIONAL INTERVAL SO THAT ALL OTHER PROPORTIONAL INTERVALS NEVER FALL BELOW THE MINIMUM  
			 */
...
int backward_index = 1000000;
	...
	if (intervalPtr->proportional && ([sender selectedRow] == 1))
	{
...
- (BOOL)validCategory:(char *)symbol
{
	...
	if ([categories usedAsCategorySymbol:symbol] != (-1))
		return (YES);
	else
	if ([template usedAsPhoneSymbol:symbol])
		return (YES);
	else
	if (!strcmp(symbol, SPECIFIER_DEF))
		return (YES);
	else
		return (NO);
}
...
{
	/*
	 * IF SPECIFIER IN SLOPE FORM, DO SLOPE TO RISE CONVERSION SO THAT THE GRAPH CAN BE DISPLAYED  
	 */
	if (currentSpecifierPtr->t_interval_mode == SLOPE_RATIO_MODE)
		[self convertRatioToRise:currentSpecifierPtr];
...
sub_intervalPtr current_interval_ptr = NULL, delete_interval_ptr = NULL;
	...
	/*
	 * MAKE SURE THAT AT LEAST ONE PROPORTIONAL INTERVAL REMAINS WHEN THE WHOLE SEGMENT IS PROPORTIONAL  
	 */
	if (currentSEIntervalProp)
	{
		current_interval_ptr = currentSEIntervalPtr->sub_intervalHead;
...
int rule_chosen;
	...
	if (rule_chosen < DUR_RULE_NORMAL || rule_chosen > DUR_RULE_T_MIDDLE)
	{
		...
		/*
		 * FORMULA SPLIT MODE, THEN RESET TO DEFAULT, RESET BUTTON TO ARBITRARY, DISABLE BUTTON  
		 */
		else
		{
			[splitModeButton setTitle:"Arbitrary Split"];
...
t_intervalPtr current_t_interval_ptr;
	...
	/*
	 * FOR EACH SPECIAL EVENT, FIND SE_INTERVAL, DELETE ALL SUB-INTERVALS, THEN SET ONE DEFAULT SUB-INTERVAL  
	 */
	current_SEPtr = currentSpecifierPtr->specialEventHead;
	for (i = 0; i < currentSpecifierPtr->number_of_special_events; i++)
	{
...
void free_se_interval();
	...
	/*
	 * FOR EACH SPECIAL EVENT, FIND SE_INTERVAL, ADJUST LINKED LIST, THEN DELETE SE_INTERVAL ITSELF  
	 */
	current_SEPtr = currentSpecifierPtr->specialEventHead;
	for (i = 0; i < currentSpecifierPtr->number_of_special_events; i++)
	{
...
sub_intervalPtr current_sub_interval_ptr, temp_sub_interval_ptr;
	...
	/*
	 * FOR EACH SPECIAL EVENT, FIND SE_INTERVAL, DELETE ALL SUB-INTERVALS, HAVE JUST ONE SUB-INTERVAL;  ADD ANOTHER SE_INTERVAL
	 * WITH ONE SUB-INTERVAL, ADJUST LINKED LIST  
	 */
	current_SEPtr = currentSpecifierPtr->specialEventHead;
	for (i = 0; i < currentSpecifierPtr->number_of_special_events; i++)
	{
...
int min;
				...
				/*
				 * IF TOO MANY SUB-INTERVALS FOR NEW LENGTH, DELETE ALL SUB-INTERVALS;  CREATE JUST ONE WITH LENGTH
				 * = NEW LENGTH  
				 */
...
int phone_length = 0, fixed_length = 0, proportional_length, excess, regression_length = 0, sample_size;
...
/*	printf("  DUR_RULE_P1\n");  */
	...
	if (g_rule->duration.rule == DUR_RULE_P2)
	{
/*	printf("  DUR_RULE_P2\n");  */
		phone_length = [phone_description getPhoneLength:phone2];
	}
	else
	if (g_rule->duration.rule == DUR_RULE_AVG)
	{
float p1, p2;
...
/*	printf("  p1 = %f  p2 = %f  t1 = %f  t2 = %f\n",p1,p2,t1,t2);  */
	...
	if (g_rule->duration.rule == DUR_RULE_T_RIGHT)
	{
float p1, p2, t1, t2;
...
/*	printf("  p1 = %f  p2 = %f  t1 = %f  t2 = %f\n",p1,p2,t1,t2);  */
	...
	if (g_rule->duration.rule == DUR_RULE_T_LEFT)
	{
float p1, p2, t1, t2;
...
/*	printf("  p1 = %f  p2 = %f  t1 = %f  t2 = %f\n",p1,p2,t1,t2);  */
	...
	if (g_rule->duration.rule == DUR_RULE_T_MIDDLE)
	{
float p1, p2, fixed;
...
/*	printf("  p1 = %f  p2 = %f  fixed = %f\n",p1,p2,fixed);  */
	...
	if (g_rule->duration.rule == DUR_RULE_FIXED)
	{
...
/*	    printf("  int1 = %d  int2 = %d  int3 = %d\n",
	...
	/*
	 * CALCULATE RISE AND ABSOLUTE VALUE FOR EACH PARAMETER, DEPENDING ON RULE AND TARGET VALUES  
	 */
...
/*	printf("  f_parameter = %s\n",current_f_parameter_ptr->symbol);  */
		...
		if (g_rule->t_interval_mode == SLOPE_RATIO_MODE)
		{
...
double m_ref, sum = 0.0;
		...
		for (j = 0; j < current_f_parameter_ptr->number_of_f_intervals; j++)
		{
			...
			if (current_f_interval_ptr->abs_value < minimum)
			{
...
void 
free_specialEventStruct(specialEventStructPtr specialEventStruct_ptr,
...
void 
free_se_interval(se_intervalPtr se_interval_ptr)
{
int i;
sub_intervalPtr current_sub_interval_ptr, temp_sub_interval_ptr;
void free_sub_interval();
...
void 
free_sub_interval(sub_intervalPtr sub_interval_ptr)
{
...
void 
free_f_parameter(f_parameterPtr f_parameter_ptr)
{
int i;
...
void 
free_f_interval(f_intervalPtr f_interval_ptr)
{
	free(f_interval_ptr);
}
...
void 
free_start_time(start_timePtr start_time_ptr)
{
	free(start_time_ptr);
}
