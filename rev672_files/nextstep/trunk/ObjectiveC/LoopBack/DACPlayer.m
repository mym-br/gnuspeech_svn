...
int i, r, protocol;
	...
	DPSAddPort(replyPort, HandleDACMessage, MSG_SIZE_MAX, self, NX_RUNMODALTHRESHOLD);
...
- stop
{
	...
	r = snddriver_stream_control(streamPort, WRITE_TAG, SNDDRIVER_ABORT_STREAM);
...
- setSamplingRate :(double)sampling_rate
{
	...
	{
		NXRunAlertPanel("Alert", "sampling rate must be %f or %f", NULL,NULL,NULL,
				SND_RATE_LOW,SND_RATE_HIGH);
...
/*handleCompleted is called whenever we get a completed message from the driver. */
- handleCompleted
{
