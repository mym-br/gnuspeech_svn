...
$Author: fedor $
$Date: 2002-12-15 05:05:11 $
$Revision: 1.2 $
$Source: /Users/dalmazio/cvsroot/gnuspeech/trillium/ObjectiveC/Monet/tube_module.old/loaderPrep.c,v $
$State: Exp $


$Log: not supported by cvs2svn $
Revision 1.1  2002/03/21 16:49:48  rao
Initial import.

 * Revision 1.1  1995/02/27  17:29:15  len
...
*     loaderPrep.c
*     
*     Converts the .lod file containing the loader code segment (assembled
*     from loader.asm), and writes out an array containing the code segment
*     to the specified .h file.  This file is included by c code, and is
*     used by that code to load the loader.asm code into high (out-of-the-
*     way) memory.  This allows user code to then be loaded into DSP memory.
*     Note that the load address and instruction count precedes the actual
*     instructions in the array.
*
******************************************************************************/
...
/*  LOCAL DEFINES  ***********************************************************/
#define OUTPUTFILE_NAME_DEF   "loader.h"
...
*                       file which contains the loader.asm DSP code.
*                       
*       arguments:      none
*                       
*                       
...
static int translate(void)
{
    ...
    /*  SET THE COLUMN AND INT COUNTS TO INITIAL VALUES  */
    currentColumn = 1;
    currentInt = 1;
    ...
    totalSize = instructionCount + 2;

    /*  WRITE OUT FILE INFORMATION  */
    fprintf(fp2, "/*\n");
    fprintf(fp2, " *   Include file:  %s\n", outputFile);
    fprintf(fp2, " *   Created by loaderPrep from %s\n", inputFile);
    fprintf(fp2, " */\n\n");

    /*  WRITE OUT SIZE IN C DECLARATION  */
    fprintf(fp2, "static int loaderCoreSize = %-d;\n", totalSize);

    /*  WRITE OUT C DECLARATION  */
    fprintf(fp2, "static int loaderCore[%-d] = {\n", totalSize);

    /*  WRITE OUT LOAD ADDRESS  */
    translateInt(loadAddress);

    /*  WRITE OUT SEGMENT SIZE (INSTRUCTION COUNT)  */
    translateInt(instructionCount);
