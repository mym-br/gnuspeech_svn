...
int bytes, messageTotal = 0, messageUsers = 0;
...
char temp[1024];
	...
	if (messageTotal<6)
	{
		...
		for (i = 0; i< messageUsers; i++)
		{
			...
			{
				sprintf(temp, "%d messages from %s,", 
					messageInfo[i].number, messageInfo[i].user);
				[mySpeech speakText:temp];
				for(j = 0; j< messageInfo[i].number; j++)
