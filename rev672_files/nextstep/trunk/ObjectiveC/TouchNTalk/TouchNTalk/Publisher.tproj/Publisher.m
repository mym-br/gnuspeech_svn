 ...
 *
 *    Last modified on "Fri Jun 10 13:47:23 1994"
 *
 ...
 * one greater then the ACTUAL number of characters in the Text object. Returns self.
 */
- publishEnglishTextDocument:aDoc
{
    ...
    buffer = [self filterControlCharacters:stream];
    [docText setText:buffer];
    free(buffer);
    stream = [docText stream];
 ...
 * and characters such as space, tab, carriage return, newline, vertical tab, or formfeed. In the
 * event that a backspace character is encountered, we filter the previous or next character as well 
 * iff it is an underscore character. We assume that a printed character was intended to be 
 * underlined. Note, it is the responsibility of the caller to free the array, once it has been used.
 * This is extremely important since large files may take up undue amounts of memory.
 */
- (char *)filterControlCharacters:(NXStream *)stream
{
	...
	ch = NXGetc(stream);
	if (NXIsPrint(ch) || NXIsSpace(ch)) {   // not a "control" character
	    buffer[j++] = ch;
