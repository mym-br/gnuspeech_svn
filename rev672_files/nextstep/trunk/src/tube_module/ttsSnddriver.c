...
$Revision: 1.1 $
$Source: /Users/dalmazio/cvsroot/gnuspeech/trillium/src/tube_module/ttsSnddriver.c,v $
$State: Exp $
		  ...
		  SNDSoundStruct *dspCore)
{
    ...
    int magic = NXSwapBigIntToHost(dspCore->magic);
    int dataLocation = NXSwapBigIntToHost(dspCore->dataLocation);
    int dataSize = NXSwapBigIntToHost(dspCore->dataSize);
    int dataFormat = NXSwapBigIntToHost(dspCore->dataFormat);
    ...
    for (i = 0; i < numberWords; i++, wordPtr++)
        core[i] = NXSwapBigIntToHost(*wordPtr);
