...
$Revision: 1.1 $
$Source: /Users/dalmazio/cvsroot/gnuspeech/trillium/src/tube_module/synthesizer_module.c,v $
$State: Exp $
...
static int await_write_done_message(int mode)
{
	...
	if (k_err == RCV_TIMED_OUT) {
		...
		if ((timeStamp() - startTime) > 6.0) {
		    ...
		    relinquish_DSP();
		    NXLogError(
		     "TTS Server:  Sound Driver failed under heavy load (p).");
		    exit(-1);
		}
	    }
...
static void write_file(void)
{
    ...
    sound.dataSize = NXSwapHostIntToBig(len);
    sound.dataFormat = NXSwapHostIntToBig(SND_FORMAT_LINEAR_16);
    sound.samplingRate = NXSwapHostIntToBig((int)output_srate);
    sound.channelCount = NXSwapHostIntToBig(numberChannels);
    sound.info[0] = '\0';
