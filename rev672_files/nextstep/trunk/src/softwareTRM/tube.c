...
$Revision: 1.1 $
$Source: /Users/dalmazio/cvsroot/gnuspeech/trillium/src/softwareTRM/tube.c,v $
$State: Exp $
...
void writeOutputToFile(char *fileName)
{
    ...
    sound.magic = NXSwapHostIntToBig(SND_MAGIC);
    sound.dataLocation = NXSwapHostIntToBig(sizeof(sound));
    sound.dataSize =
	NXSwapHostIntToBig(channels * numberSamples * sizeof(short));
    sound.dataFormat = NXSwapHostIntToBig(SND_FORMAT_LINEAR_16);
    sound.samplingRate = NXSwapHostIntToBig((int)outputRate);
    sound.channelCount = NXSwapHostIntToBig(channels);
    sound.info[0] = sound.info[1] = sound.info[2] = sound.info[3] = '\0';
	...
	for (i = 0, j = 0; i < numberSamples; i++) {
	    ...
	    fwrite(&data, sizeof(data), 1, fd);
	    data = NXSwapHostShortToBig((short)rint(sample * rightScale));
	    fwrite(&data, sizeof(data), 1, fd);
	}
    }
