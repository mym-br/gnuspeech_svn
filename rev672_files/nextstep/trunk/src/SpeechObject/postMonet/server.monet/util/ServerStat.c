...
#import <string.h>
#import <sys/file.h>
#import "../headerFiles/SpeechMessages.h"
#import "../headerFiles/MessageStructs.h"
...
struct string_msg message;
	...
	if (initiate_connection(&SpeechIdentifier) == 0)
	{
		printf("Connected to server through port: %d\n", SpeechIdentifier);
	        ...
	        send_simple_message(outPort, localPort, VERSION, SpeechIdentifier);
		receive_string_message(localPort, &message);
		printf("Version is: %s\n", message.data);
		vm_deallocate(task_self(), (vm_address_t)message.data,strlen(message.data)+2);

	        send_simple_message(outPort, localPort, DIAGNOSIS, SpeechIdentifier);
		receive_string_message(localPort, &message);
		printf("%s\n", message.data);
		vm_deallocate(task_self(), (vm_address_t)message.data,strlen(message.data)+2);
