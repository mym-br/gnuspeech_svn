...
static int
  ...
  fractional_digits_pos[FRACTIONAL_DIGITS_MAX], ordinal_pos[2],
...
void initial_parse()
{
    ...
    degenerate = integer_digits = fractional_digits = commas = decimal = 0;
    dollar = percent = negative = positive = ordinal = clock = slash = 0;
...
int error_check(int mode)
{
	...
	FOLLOWING:  . $ % + / ( ) blank  */
    if (degenerate || decimal > 1 || dollar > 1 || percent > 1 ||
    ...
    if (left_zero_pad && (integer_digits > 1) && (!clock) && (!telephone))
	return (DEGENERATE);
	...
	if ((!integer_digits) || (!fractional_digits) ||
	      decimal || dollar || commas || clock || ordinal)
	    return (DEGENERATE);
    }
	...
	if (integer_digits == 3) {
	    if ((word_length != 4) || (clock_pos[0] != 1))
		return (DEGENERATE);
	    hour[1] = '0';
	    ...
	    if ((word_length != 5) || (clock_pos[0] != 2))
		return (DEGENERATE);
	    ...
	    if ((word_length != 7) || (clock_pos[0] != 1) || (clock_pos[1] != 4))
	        return (DEGENERATE);
	    hour[1] = '0';
	    hour[2] = word[integer_digits_pos[0]];
	    minute[1] = word[integer_digits_pos[1]];
	    ...
	    if ((word_length != 8) || (clock_pos[0] != 2) || (clock_pos[1] != 5))
	        return (DEGENERATE);
	    hour[1] = word[integer_digits_pos[0]];
	    hour[2] = word[integer_digits_pos[1]];
	    minute[1] = word[integer_digits_pos[2]];
	...
	if (hours > 24 || minutes > 59 || secs > 59)
	         return (DEGENERATE);
	military = (hours >= 1 && hours <= 12 && (!left_zero_pad)) ? NO : YES;
	}
    }
	...
	if (decimal && (decimal_pos != (commas_pos[commas - 1] + 4)))
	    return (DEGENERATE);
	...
	if ((!negative) && (!positive) && (dollar_pos != 0))
	    return (DEGENERATE);
	for (i = integer_digits - 1, dollar_plural = dollar_nonzero = NO;
	                                                        i >= 0; i--) {
	...
	for (i = 0, cents_plural = YES, cents_nonzero = NO;
    ...
    int mode;
{
	...
	if (left_zero_pad)
	    strcat(output, OH);
	process_triad(hour, output, NO, NO, NO, NO, NO);
        /*  MINUTE  */
	if ((minute[1] == '0') && (minute[2] == '0')) {
	    ...
	    if ((minute[1] == '0') && (minute[2] != '0'))
		strcat(output, OH);
	    process_triad(minute, output, NO, NO, NO, NO, NO);
	}
	/*  SECOND  */
	if (seconds) {
	    strcat(output, AND);
		...
		strcat(output, ZERO);
	      ...
	      process_triad(second, output, NO, NO, NO, NO, NO);
	...
	for (i = 0; i < 3; i++)
	    process_digit(*(word+(*(integer_digits_pos+i))), output, NO, NO, NO);
	strcat(output, PAUSE);
	for (i = 3; i < 7; i++)
	    process_digit(*(word+(*(integer_digits_pos+i))), output, NO, NO, NO);
	return (output);
	...
	process_digit(word[integer_digits_pos[0]], output, NO, NO, NO);
	if ((word[integer_digits_pos[1]] != '0') &&
	      (word[integer_digits_pos[2]] == '0') &&
	      (word[integer_digits_pos[3]] == '0')) {
	    process_digit(word[integer_digits_pos[1]], output, NO, NO, NO);
	    strcat(output, HUNDRED);
	} else {
	    strcat(output, PAUSE);
	    for (i = 1; i < 4; i++)
	...
	process_triad(triad, output, NO, NO, NO, NO, NO);
	if ((word[integer_digits_pos[2]] == '0') &&
	      (word[integer_digits_pos[3]] == '0'))
	    strcat(output, HUNDRED);
	else if (word[integer_digits_pos[2]] == '0') {
	    strcat(output, OH);
	    process_digit(word[integer_digits_pos[3]], output, NO, NO, NO);
	} else {
	    triad[0] = '0';
	...
	if ((word[integer_digits_pos[0]] == '0') && dollar)
	  ;
	    ...
	    process_digit(word[integer_digits_pos[0]],output,ordinal,NO,NO);
	ordinal_plural = (word[integer_digits_pos[0]] == '1') ? NO : YES;
    }
	...
	for (i = 0; i < fractional_digits; i++)
	    process_digit(word[fractional_digits_pos[i]], output, NO, NO, NO);
    }
	...
	if (process_triad(triad, output, NO, NO, NO, NO, NO) == NONZERO) {
	    if (cents_plural)
		strcat(output, CENTS);
	    else
		strcat(output, CENT);
