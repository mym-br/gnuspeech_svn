...
$Revision: 1.1 $
$Source: /Users/dalmazio/cvsroot/gnuspeech/trillium/src/SpeechObject/postMonet/objC.monet/TextToSpeechPron.m,v $
$State: Exp $
...
@implementation TextToSpeech(TextToSpeechPron)
...
- (const char *)pronunciation:(const char *)word:(short *)dict:(int)password
{
    ...
    *dict = 0;
	...
	if (send_string_message(outPort, localPort, GET_PRON,
				SpeechIdentifier, word) != SEND_SUCCESS) {
...
- (const char *)linePronunciation:(const char *)line:(int)password
{
    ...
    if ((password == 0xdeafbabe) && (strlen(line) > 0)) {
      restart:
	/*  QUERY SERVER FOR PRONUNCIATION  */
	if (send_string_message(outPort, localPort, GET_LINE_PRON,
				SpeechIdentifier, line) != SEND_SUCCESS) {
	    if ([self restartServer] == TTS_SERVER_RESTARTED)
		goto restart;
	    else
