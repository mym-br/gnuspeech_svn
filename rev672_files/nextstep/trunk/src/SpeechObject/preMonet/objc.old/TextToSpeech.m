...
$Revision: 1.1 $
$Source: /Users/dalmazio/cvsroot/gnuspeech/trillium/src/SpeechObject/preMonet/objc.old/TextToSpeech.m,v $
$State: Exp $
...
- read:(NXTypedStream *)stream
{
	...
	NXReadTypes(stream, "fiifff", &speed, &intonation,
		    &voiceType, &pitchOffset, &volume, &balance);
	NXReadArray(stream, "s", 4, dictionaryOrder);
	NXReadArray(stream, "c", MAXPATHLEN, appDictPath);
	NXReadArray(stream, "c", MAXPATHLEN, userDictPath);
	...
	realTimeMessagesDestination = NXReadObject(stream);
	NXReadTypes(stream, ":i", &realTimeMessagesSelector, &realTimeMessages);
	NXReadTypes(stream, "fffi", &vtlOffset, &breathiness, &sampleRate, &channels);
    }
...
- write:(NXTypedStream *)stream
{
    ...
    NXWriteTypes(stream, "fiifff", &speed, &intonation,
		 &voiceType, &pitchOffset, &volume, &balance);
    ...
    NXWriteTypes(stream, "fffi", &vtlOffset, &breathiness, &sampleRate, &channels);
