...
int parser(const char *input, char **output)
{
  ...
  if (stream2 != NULL) {
    NXCloseMemory(stream2, NX_TRUNCATEBUFFER);
    stream2 = NULL;
  }
  ...
  if ((error = final_conversion(stream1, stream1_length, stream2, &stream2_length))
      != TTS_PARSER_SUCCESS) {
    NXCloseMemory(stream1, NX_TRUNCATEBUFFER);
    NXCloseMemory(stream2, NX_TRUNCATEBUFFER);
    stream2 = NULL;
    return(error);
  }
	  ...
	  switch(current_state) {
	    ...
	    case STATE_MEDIAL_PUNC:
		  ...
		  if (shift_silence(input, i, stream1_length, mode, stream2))
		    last_word_end = NXTell(stream2);
		  else if (another_word_follows(input, i, stream1_length, mode)) {
		    if (!strcmp(word,","))
		      NXPrintf(stream2,"%s ", MEDIAL_PAUSE);
		    else
		      NXPrintf(stream2,"%s ", LONG_MEDIAL_PAUSE);
		  }
		case STATE_SILENCE:
		  NXPrintf(stream2,"%s ",TONE_GROUP_BOUNDARY);
		  prior_tonic = TTS_FALSE;
...
static int illegal_slash_code(char *code)
{
  ...
  static char *legal_code[] = {CHUNK_BOUNDARY,TONE_GROUP_BOUNDARY,FOOT_BEGIN,
			       ...
			       TG_STATEMENT,TG_EXCLAMATION,TG_QUESTION,
...
static void insert_chunk_marker(NXStream *stream, long insert_point, char tg_type)
{
  ...
  NXCloseMemory(temp_stream, NX_TRUNCATEBUFFER);
}
