...
/*===========================================================================
	...
	File modified June 16, 1992.  init_dict has been changed.  The dictionary
		no longer needs to be at some absolute path.  The path is
		passed as a parameter to init_dict()
...
unsigned int hash[4],hashval;			/* Hashes are unsigned */

extern int size[];				/* From index file */
extern unsigned int *trees[];			/* From index file */
static char final[256];				/* Resulting buffer */
...
static unsigned int hashcalc(char *word);
static int get_offset(int hash, int number);
static void decrypt(int offset, char *dummy, char *final);
...
int init_dict(const char *main_dictionary_path)
{
        if ((fp = NXMapFile(main_dictionary_path, NX_READONLY)) == NULL)
            return(MD_FAILURE);
        else
            return(MD_SUCCESS);
...
static unsigned int hashcalc(word)
/*===========================================================================
...
static int get_offset(hash,number)
/*===========================================================================
...
static void decrypt(offset, dummy, final)
int offset;
char *dummy, *final;
{
int temp;
...
int temp;
        ...
        NXSeek(fp,0,NX_FROMSTART);               /* seek to word */
#ifndef ENC
        /* Read version string */
        while(1)
        {
                temp = NXGetc(fp);
                if (temp == 0x0a) break;
                tempstring[i++] = (char) temp;
                if (i>=255) break;
        }
#endif
#ifdef ENC
        /* Decrypt version string */
        while(1)
        {
                temp = NXGetc(fp);
                temp = temp-buffer[index];
