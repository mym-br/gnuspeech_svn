...
mig_internal novalue _Xdsp_set_short_swapped_return
	(msg_header_t *InHeadP, msg_header_t *OutHeadP)
{
	typedef struct {
		msg_header_t Head;
	...
	register Request *In0P = (Request *) InHeadP;
	register Reply *OutP = (Reply *) OutHeadP;
	extern kern_return_t dsp_set_short_swapped_return (port_t dspdriver_port, port_t owner_port, int regionTag, int wordCount, port_t reply_port, int unit);
...
#define	label_punt0
	...
	(void) dsp_set_short_swapped_return(In0P->Head.msg_request_port, In0P->owner_port, In0P->regionTag, In0P->wordCount, In0P->reply_port, In0P->unit);
	OutP->RetCode = MIG_NO_REPLY;
#ifdef	label_punt0
#undef	label_punt0
punt0:
	...
	(msg_header_t *InHeadP, msg_header_t *OutHeadP)
{
	...
	if ((InP->msg_id > 526) || (InP->msg_id < 500))
		return FALSE;
	else {
		...
		static const SERVER_STUB_PROC routines[] = {
			...
			_Xdsp_set_messaging,
			_Xdsp_queue_page,
			_Xdsp_set_short_swapped_return,
		};
