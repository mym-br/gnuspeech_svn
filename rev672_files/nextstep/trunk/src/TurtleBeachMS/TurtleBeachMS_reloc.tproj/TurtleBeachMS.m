...
-resetDSP:(char)resetOn
{
	...
	outb(MULTISOUND_WAIT(baseIO),0);  // changed to 3 wait states, not 4
	...
	/*  MAKE SURE THAT MESSAGING IS OFF, AND OUTPUT QUEUE IS RESET  */
	[self setMessagingOn:0];
	[self resetOutputQueue];

	/*  MAKE SURE DMA OUT VARIABLES ARE RESET  */
	[self resetDMAOut];	

	/*  MAP THE INTERRUPT;  DONE HERE SINCE TXDE GUARANTEED SET TO 1  */
	[self mapDSPInterrupt];
    }
