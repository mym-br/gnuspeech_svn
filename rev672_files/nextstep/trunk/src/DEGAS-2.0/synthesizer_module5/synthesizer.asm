...
DATA_TABLE_SIZE		equ	11	; make sure this (number+19) is matched in
					; the 68040 (30)
...
JUNK_SKIP	equ	2		; table size (32) - actual data (30) = 2
...
write_sample_stereo_bal
	...
	move	y:BALANCE,x0		; x0 contains R scaling
	sub	x0,b	 a,y0		; L scaling = 1.0 - R scaling, y0 contains signal
	mpy	x0,y0,a	 b,x1		; scale R channel, x1 contains L scaling
	mpy	x1,y0,a	 a,x:(r7)+	; scale L channel, put R value in dma-out buffer
	move	a,x:(r7)+		; put L output value into dma-out buffer
...
update_datatable
	...
	move	#@cvf(1.0/44.0),y0	; put 1/44 into y0 (interpolation factor)
					; make sure y0 is not altered below
	move	x:x_index2,a
	move	#>1,y1
	sub	y1,a	#>INCREMENT1,b	; index2--
	jne	_continue
	  add	y1,b	#>INCREMENT2,a
	  move	#@cvf(1.0/45.0),y0	; put 1/45 into y0 when using larger
					; increment (make sure y0 not altered)
_continue
...
;; LOAD FILTER COEFFICIENTS DIRECTLY INTO X FILTER MEMORY
	move	y:(r5)+,x1
	move	x1,x:XC1MEMC		; move coefficient into x memory
	move	y:(r5)+,x1
	move	x1,x:XC1MEMB		; move coefficient into x memory
	move	y:(r5)+,x1
	move	x1,x:XC1MEMA		; move coefficient into x memory

	move	y:(r5)+,x1
	move	x1,x:XC2MEMC		; move coefficient into x memory
	move	y:(r5)+,x1
	move	x1,x:XC2MEMB		; move coefficient into x memory
	move	y:(r5)+,x1
	move	x1,x:XC2MEMA		; move coefficient into x memory

	move	y:(r5)+,x1
	move	x1,x:XC3MEMC		; move coefficient into x memory
	move	y:(r5)+,x1
	move	x1,x:XC3MEMB		; move coefficient into x memory
	move	y:(r5)+,x1
	move	x1,x:XC3MEMA		; move coefficient into x memory
	move	y:(r5)+,x1

	move	x1,x:XC4MEMC		; move coefficient into x memory
	move	y:(r5)+,x1
	move	x1,x:XC4MEMB		; move coefficient into x memory
	move	y:(r5)+,x1
	move	x1,x:XC4MEMA		; move coefficient into x memory
	move	y:(r5)+,x1

	move	x1,x:XFRMEMC		; move coefficient into x memory
	move	y:(r5)+,x1
	move	x1,x:XFRMEMB		; move coefficient into x memory
	move	y:(r5)+,x1
	move	x1,x:XFRMEMA		; move coefficient into x memory
...
;; CALCULATE DELTA VALUE FOR LINEAR AMPLITUDE INTERPOLATION
	...
	move	b,x0
	mpyr	x0,y0,b			; y0 preserved from above
...
resonator
	clr	b	x:(r0)+,x0  y:(r4)+,y0	; put C in x0, put y(n-2) in y0
	mac	x0,y0,b x:(r0)+,x0  y:(r4)+,y0	; b = c*y(n-2), put B(frac) in
						; x0, y(n-1) in y0
	add	y0,b				; b += 1*y(n-1)
	mac	x0,y0,b	x:(r0)+,x0  a,y0	; b += B(frac)*y(n-1), put
						; A(frac) in x0, input in y0
	add	y0,b				; b += 1*x(n)  ; x(n)=input
	macr	x0,y0,b				; b += A(frac)*x(n)
