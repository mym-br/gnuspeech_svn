...
- (void)drawRect:(NSRect)rect // This method over-rides ChartView
{
	...
	if (normalTestState == 0 && analysisDataExists == YES) {
		NSLog(@"Spectrum.m:156, Normal analysis (flag is %d) and analysisDataExists YES (flag is %d)", normalTestState, analysisDataExists);		
	}
	
	else { // Either normalTestState is 1 or analysisDataExists is NO

		if (normalTestState == 0) {
			NSLog(@"Spectrum.m:159, analysisDataExists NO (flag is %d)", analysisDataExists);
			int runButtonState = [runButton state];
			if (runButtonState == 1) {
				tube_getCircBuff(analysisData);
			}
			...
			for (i = 0; i < TEST_DATA_SIZE; i++)
				...
				analysisDataExists = YES; // #### was YES
				
				} // End of testWave data tranfer loop
			
			} //End normalTestState i 1	

	} // End of actions when normalTestState is 1 &/OR analysisDataExists is NO
	...
	float *envelopeData = (float *)calloc(samplingWindowSize / 2, sizeof(float));
