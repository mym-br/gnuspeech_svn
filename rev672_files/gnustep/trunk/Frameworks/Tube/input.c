...
TRMInputData *parseInputFile(const char *inputFile)
{
    int i;
    FILE *fp;
    char line[128];
    int numberInputTables = 0;
    TRMInputData data, *result;
    ...
    result = (TRMInputData *)malloc(sizeof(TRMInputData));
    if (result == NULL) {
        fprintf(stderr, "Couldn't malloc() TRMData.\n");
        return NULL;
    }

    memcpy(result, &data, sizeof(TRMInputData));
...
void addInput(TRMInputData *data, double glotPitch, double glotVol, double aspVol, double fricVol,
...
void printControlRateInputTable(TRMInputData *data)
{
    INPUT *ptr;
    int index;
