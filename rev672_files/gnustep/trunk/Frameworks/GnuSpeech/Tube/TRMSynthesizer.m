						...
						void *userData)
{
	...
	unsigned long transferBytes = (frameCount << 1) * data->channels;
	unsigned long availableBytes = data->size - data->position;
	
	if (transferBytes < availableBytes) {
		memcpy(output, data->buffer + data->position, transferBytes);
		data->position += transferBytes;
		return paContinue;
	} else {
		memcpy(output, data->buffer + data->position, availableBytes);
		if (transferBytes > availableBytes) {
			// Fill with zeros.
			memset(output + availableBytes, 0, transferBytes - availableBytes);
		}
...
- (void)resetPaCallbackData
{
	paCallbackData.buffer = NULL;
	paCallbackData.size = 0;
	paCallbackData.position = 0;
	paCallbackData.channels = 0;
}
...
{	
    ...
    inputData = (TRMInputData *)malloc(sizeof(TRMInputData));
    if (inputData == NULL) {
        NSLog(@"Failed to malloc TRMInputData.");
        [super release];
        return nil;
    }
...
- (void)convertSamplesIntoData:(TRMSampleRateConverter *)sampleRateConverter;
{
    ...
    if (sampleRateConverter->maximumSampleValue == 0.0) {
        NSLog(@"Warning: sampleRateConverter->maximumSampleValue == 0.0.");
        //NSBeep();
        scale = 0.0;
    } else {
...
- (void)startPlaying;
{
	...
	if (paInitialized == NO) {
		NSLog(@"Error: Could not start playing: the sound device is not initialized.");
		return;
	}
	if (paStream != NULL) {
		NSLog(@"Error: Could not start playing: there is an open stream.");
		return;
	}

	// Prepare the callback data.
	[self resetPaCallbackData];
	paCallbackData.buffer = [soundData bytes];
	paCallbackData.size = [soundData length];
	paCallbackData.channels = inputData->inputParameters.channels;

	// Open a stream.
	PaStreamParameters outputParameters;
	outputParameters.device = Pa_GetDefaultOutputDevice();
	outputParameters.channelCount = inputData->inputParameters.channels;
	...
	err = Pa_OpenStream(
				...
				NULL, // no input
				&outputParameters,
				inputData->inputParameters.outputRate,
				paFramesPerBufferUnspecified,
				paNoFlag,
				paStreamCallback,
				&paCallbackData);
	if (err != paNoError) {
		NSLog(@"Error: Could not open the sound stream. Cause: %s", Pa_GetErrorText(err));
		[self resetPaCallbackData];
		paStream = 0;
		return;
	}
	...
	if (err != paNoError) {
		NSLog(@"Error: Could not start the sound stream. Cause: %s", Pa_GetErrorText(err));
		[self resetPaCallbackData];
		return;
	}
...
{
#ifdef GNUSTEP
	if (paInitialized == YES) {
		// If a stream is open, close it.
		if (paStream != NULL) {
			...
			if (err != paNoError) {
				NSLog(@"Error: Could not close the sound stream. Cause: %s", Pa_GetErrorText(err));
				return NO;
			}
			paStream = NULL;
		}
...
- (void)setupSoundDevice;
{
	...
	if (err != paNoError) {
		NSLog(@"Error: Could not initialize the sound device. Cause: %s", Pa_GetErrorText(err));
		Pa_Terminate();
		return;
	}
	...
	if (devIdx == paNoDevice) {
		NSLog(@"Error: No audio output device is available. API: %s.", Pa_GetHostApiInfo(Pa_GetDefaultHostApi())->name);
		Pa_Terminate();
		return;
	}
